...
,Salary int
,SuperSSN varchar(20) null
,DepNo int null
,FOREIGN KEY(SuperSSN)references employee on delete cascade on update cascade
,FOREIGN KEY(DepNo)references department on delete cascade on update cascade);
...
alter table department add foreign key(SSN) references employee on delete cascade on update cascade;
...
alter table dept_locations add foreign key(DepNo) references department on delete cascade on update cascade;
...
create table dependent(SSN varchar(20),
...
create table works_on(SSN varchar(20),PNumber int,Hours numeric(8,4),
primary key(SSN,PNumber),
foreign key(PNumber) references project on delete cascade on update cascade);
...
select min(salary),Dname from employee,department where employee.DepNo=department.DepNo group by Dname;
select avg(salary),Dname from employee,department where employee.DepNo=department.DepNo group by Dname having avg(salary) > 40000;
select count(Fname) from employee where DepNo=6;
select count(Fname),Dname from employee ,department where Fname like 'J%' and department.DepNo=employee.DepNo group by Dname;
select count(Pname),Dname from project,department where project.DepNo=department.DepNo group by Dname;
select * from project;
